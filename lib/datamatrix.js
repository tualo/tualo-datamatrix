...
Datamatrix.prototype.aLogTab = [
                        ...
                        150, 1];
Datamatrix.prototype.champGaloisMult = function(a, b){  // MULTIPLICATION IN GALOIS FIELD GF(2^8)
	if(!a || !b) return 0;
	return this.aLogTab[(this.logTab[a] + this.logTab[b]) % 255];
}
Datamatrix.prototype.champGaloisDoub=function(a, b){  // THE OPERATION a * 2^b IN GALOIS FIELD GF(2^8)
	if (!a) return 0;
	if (!b) return a;
	return this.aLogTab[(this.logTab[a] + b) % 255];
}
Datamatrix.prototype.champGaloisSum=function(a, b){ // SUM IN GALOIS FIELD GF(2^8)
	return a ^ b;
}
Datamatrix.prototype.selectIndex=function(dataCodeWordsCount, rectangular){ // CHOOSE THE GOOD INDEX FOR TABLES
...
Datamatrix.prototype.encodeDataCodeWordsASCII=function(text) {
	var dataCodeWords = [];
	var n = 0;
	var len = text.length;
	var reg = /[0-9]/;
...
// addPadCW
Datamatrix.prototype.addPadCW=function(tab, from, to){    
...
Datamatrix.prototype.calculSolFactorTable=function(solomonCWCount){ // CALCULATE THE REED SOLOMON FACTORS
...
Datamatrix.prototype.addReedSolomonCW=function(nSolomonCW, coeffTab, nDataCW, dataTab, blocks){ // Add the Reed Solomon codewords
	var temp = 0;
	var j=0;
	var i=0;
...
Datamatrix.prototype.getBits=function(entier){ // Transform integer to tab of bits
	var bits = [];
	for (i=0; i<8; i++){
...
Datamatrix.prototype.next=function(etape, totalRows, totalCols, codeWordsBits, datamatrix){ // Place codewords into the matrix
...
Datamatrix.prototype.patternShapeStandard=function(datamatrix, bits, row, col, totalRows, totalCols){ // Place bits in the matrix (standard or special case)
...
Datamatrix.prototype.patternShapeSpecial1=function(datamatrix, bits, totalRows, totalCols ){
...
Datamatrix.prototype.patternShapeSpecial2=function(datamatrix, bits, totalRows, totalCols ){
...
Datamatrix.prototype.patternShapeSpecial3=function(datamatrix, bits, totalRows, totalCols ){
	datamatrix = this.placeBitInDatamatrix(datamatrix, bits[0], totalRows-3,  0, totalRows, totalCols);
	datamatrix = this.placeBitInDatamatrix(datamatrix, bits[1], totalRows-2,  0, totalRows, totalCols);
	datamatrix = this.placeBitInDatamatrix(datamatrix, bits[2], totalRows-1,  0, totalRows, totalCols);
	datamatrix = this.placeBitInDatamatrix(datamatrix, bits[3], 0, totalCols-2, totalRows, totalCols);
...
Datamatrix.prototype.patternShapeSpecial4=function(datamatrix, bits, totalRows, totalCols ){
	datamatrix = this.placeBitInDatamatrix(datamatrix, bits[0], totalRows-1,  0, totalRows, totalCols);
...
Datamatrix.prototype.placeBitInDatamatrix=function(datamatrix, bit, row, col, totalRows, totalCols){ // Put a bit into the matrix
	if (row < 0) {
		row += totalRows;
		col += 4 - ((totalRows+4)%8);
	}
...
Datamatrix.prototype.addFinderPattern=function(datamatrix, rowsRegion, colsRegion, rowsRegionCW, colsRegionCW){ // Add the finder pattern
...
Datamatrix.prototype.getDigit=function(text, rectangular){
