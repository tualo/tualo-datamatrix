var array_fill=function(start,len,value){
	var res = [];
...
var Matrix = function(rows,cols,fill){
	this.cols=cols;
	this.rows=rows;
	this.data = '';
...
Matrix.prototype.assigned=function(row,col){
//	console.log('assigned '+row+' '+col+': '+ (this.get(row,col)===''?'Y':'N'));
	return this.get(row,col)!=='';
}

var BarcodeDatamatrix = function(){

}
BarcodeDatamatrix.prototype.encoding = [
...
BarcodeDatamatrix.prototype.aLogTab = [
                        ...
                        150, 1];
BarcodeDatamatrix.prototype.champGaloisMult = function(a, b){  // MULTIPLICATION IN GALOIS FIELD GF(2^8)
	if(!a || !b) return 0;
	return this.aLogTab[(this.logTab[a] + this.logTab[b]) % 255];
}
BarcodeDatamatrix.prototype.champGaloisDoub=function(a, b){  // THE OPERATION a * 2^b IN GALOIS FIELD GF(2^8)
	if (!a) return 0;
	if (!b) return a;
	return this.aLogTab[(this.logTab[a] + b) % 255];
}
BarcodeDatamatrix.prototype.champGaloisSum=function(a, b){ // SUM IN GALOIS FIELD GF(2^8)
	return a ^ b;
}
BarcodeDatamatrix.prototype.selectIndex=function(dataCodeWordsCount, rectangular){ // CHOOSE THE GOOD INDEX FOR TABLES
...
BarcodeDatamatrix.prototype.encodeDataCodeWordsASCII=function(text) {
	var dataCodeWords = [];
	var n = 0;
	var len = text.length;
	var reg = /[0-9]/;
...
// addPadCW
BarcodeDatamatrix.prototype.addPadCW=function(tab, from, to){    
...
BarcodeDatamatrix.prototype.calculSolFactorTable=function(solomonCWCount){ // CALCULATE THE REED SOLOMON FACTORS
	var g = array_fill(0, solomonCWCount+1, 1);
...
BarcodeDatamatrix.prototype.addReedSolomonCW=function(nSolomonCW, coeffTab, nDataCW, dataTab, blocks){ // Add the Reed Solomon codewords
	var temp = 0;
	var j=0;
	var i=0;
...
BarcodeDatamatrix.prototype.getBits=function(entier){ // Transform integer to tab of bits
	var bits = [];
	for (i=0; i<8; i++){
...
BarcodeDatamatrix.prototype.next=function(etape, totalRows, totalCols, codeWordsBits, datamatrix){ // Place codewords into the matrix
...
BarcodeDatamatrix.prototype.patternShapeStandard=function(datamatrix, bits, row, col, totalRows, totalCols){ // Place bits in the matrix (standard or special case)
...
BarcodeDatamatrix.prototype.patternShapeSpecial1=function(datamatrix, bits, totalRows, totalCols ){
...
BarcodeDatamatrix.prototype.patternShapeSpecial2=function(datamatrix, bits, totalRows, totalCols ){
...
BarcodeDatamatrix.prototype.patternShapeSpecial3=function(datamatrix, bits, totalRows, totalCols ){
	datamatrix = this.placeBitInDatamatrix(datamatrix, bits[0], totalRows-3,  0, totalRows, totalCols);
	datamatrix = this.placeBitInDatamatrix(datamatrix, bits[1], totalRows-2,  0, totalRows, totalCols);
	datamatrix = this.placeBitInDatamatrix(datamatrix, bits[2], totalRows-1,  0, totalRows, totalCols);
	datamatrix = this.placeBitInDatamatrix(datamatrix, bits[3], 0, totalCols-2, totalRows, totalCols);
...
BarcodeDatamatrix.prototype.patternShapeSpecial4=function(datamatrix, bits, totalRows, totalCols ){
	datamatrix = this.placeBitInDatamatrix(datamatrix, bits[0], totalRows-1,  0, totalRows, totalCols);
...
BarcodeDatamatrix.prototype.placeBitInDatamatrix=function(datamatrix, bit, row, col, totalRows, totalCols){ // Put a bit into the matrix
	if (row < 0) {
		row += totalRows;
		col += 4 - ((totalRows+4)%8);
	}
...
BarcodeDatamatrix.prototype.addFinderPattern=function(datamatrix, rowsRegion, colsRegion, rowsRegionCW, colsRegionCW){ // Add the finder pattern
...
BarcodeDatamatrix.prototype.getDigit=function(text, rectangular){
...
exports.Datamatrix = BarcodeDatamatrix;